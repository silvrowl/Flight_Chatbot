...
Complete_Sample = (
    SAMPLES_TEXT_1 + SAMPLES_TEXT_2 + SAMPLES_TEXT_3 + SAMPLES_TEXT_4 + SAMPLES_TEXT_5
)
...
Complete_Sample = SAMPLES_TEXT_2
...
def text2int(textnum, numwords={}):
    ...
    for word in textnum.split():
        if word in ORDINAL_WORDS:
            ...
            for ending, replacement in ORIDINAL_ENDINGS:
...
def data_cleaning(sentence_list):
    ...
    for cnt in np.arange(0, len(sentence_list)):
        sentence_list[cnt][0] = sentence_list[cnt][0].replace(
...
def match_score(tagged_word):
    ...
    match_df = pd.DataFrame(
        data_tuples, columns=["city", "city_score", "state", "state_score", "code"]
    )
...
def match_score_list(tagged_word):
    ...
    if match_sorted["city_score"].max() > 90:
        options_1 = match_sorted[
            match_sorted["city_score"] == match_sorted["city_score"].max()
        ]
        return options_1.head(5)

    elif (
        match_sorted["city_score"].max() > 75
        and match_sorted["city_score"].max() > state_sorted["state_score"].max()
    ):
        options_1 = match_sorted[
            match_sorted["city_score"] == match_sorted["city_score"].max()
        ]
        ...
        if state_sorted["state_score"].max() > 75:
            options_2 = state_sorted[
                state_sorted["state_score"] == match_sorted["state_score"].max()
            ]
...
def dates_ner(words_tagged):
    ...
    for cnt in np.arange(0, len(words_tagged)):
        ...
        if words_tagged[cnt][0] in MONTH_LOWER:
            if (
                words_tagged[cnt][1] == "NN"
                or words_tagged[cnt][1] == "NNS"
                or words_tagged[cnt][1] == "JJ"
            ):
...
# travel_chat_preproc = word_preproc(SAMPLES_TEXT_1)
